...
from test_handler import index,COOKIE_NAME
...
async def auth_factory(app,handler):
    async def auth(request):
        ...
        #判断是否为管理员
        #用mysql命令将其中一个用户设置成管理员即可update users set admin=1 where name="刘帅";
        if request.path.startswith('/manage/') and (request.__user__ is None or not request.__user__.admin):#本来是not request.__user__.admin
...
async def init(loop):
    ...
    add_routes(app, 'test_handler')  #调入测试程序test_handler.py
    # 添加静态文件所在地址
    add_static(app)
    # 启动
